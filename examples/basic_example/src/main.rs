...
fn main() {
    ...
    // Playlist tracks 
    let playlist_uri = uri_from_str(String::from("spotify:user:1111580463:playlist:3wPPzGGWWqIWWHu3TuilEh")).unwrap();
    let playlist_tracks = playlist_get_tracks(&spotify_client, spotify_token.clone(), playlist_uri.clone());

    println!("Number of tracks: {}", playlist_tracks.items.len());
    println!("First track: {} by {}", playlist_tracks.items[0].track.name, playlist_tracks.items[0].track.artists[0].name);
}
